...
from .models import (
    Courses, Categories, Partners, Gallery, Blog, 
    FrequentlyAskQuestions,  
    Topics, CourseFrequentlyAskQuestions,
    CourseLearnerReviews, CareerOpportunities, Skillyouwillgain,
    Whatyouwilllearn, Whatyouwillbuild, AboutCourseOwner,
...
def download_badge_image(request, student_id, course_id, rank):
    result = Result.objects.filter(student__id=student_id, exam__id=course_id).order_by('-marks').first()
...
class DashboardCourses(LoginRequiredMixin, HitCountDetailView, DetailView):
    ...
    def get_queryset(self):
        # Print all courses being fetched
        qs = Courses.objects.all()
        print("🔹 All Courses in get_queryset():", qs)
        return qs

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        course = self.get_object()  # The current course
        context['coursess'] = Courses.objects.all().order_by('created')
