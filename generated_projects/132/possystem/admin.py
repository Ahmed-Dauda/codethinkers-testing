from .models import Sale, Attendant, Expense, Category
...
class SaleAdmin(ExportAdminMixin, admin.ModelAdmin):
    list_display = ('amount', 'date', 'attendant', 'category')
    list_filter = ('date', 'attendant', 'category')
    search_fields = ('attendant__name', 'category__name')
    list_per_page = 25
    list_select_related = ('attendant', 'category')
    readonly_fields = ('created_at', 'updated_at')
    ordering = ('-date',)

    def get_queryset(self, request):
        return super().get_queryset(request).select_related('attendant', 'category').order_by('-date')

@admin.register(Attendant)
class AttendantAdmin(ExportAdminMixin, admin.ModelAdmin):
...
class ExpenseAdmin(ExportAdminMixin, admin.ModelAdmin):
    ...
    list_filter = ('date',)
    search_fields = ()  # No searchable fields left
    list_per_page = 25
    readonly_fields = ('created_at', 'updated_at')
    ordering = ('-date',)
